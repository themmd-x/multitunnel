...
import (
	...
	"github.com/TheMMD-X/multitunnel/libs/gophertunnel/v121130/minecraft/protocol/login"
	"github.com/TheMMD-X/multitunnel/libs/gophertunnel/v121130/minecraft/protocol/packet"
)
...
func (network) DialContext(context.Context, string) (net.Conn, error) {
	return nil, errors.New("v121130: the multitunnel network cannot dial")
}

func (network) PingContext(context.Context, string) ([]byte, error) {
	return nil, errors.New("v121130: the multitunnel network cannot ping")
}

func (network) Listen(address string) (minecraft.NetworkListener, error) {
	f, ok := feeders.LoadAndDelete(address)
	if !ok {
		return nil, fmt.Errorf("v121130: no feeder registered under %q", address)
	}
	return f.(*feeder), nil
}
...
func Disconnect(conn net.Conn, message string) error {
	...
	if !ok {
		return fmt.Errorf("v121130: disconnect: unexpected connection type %T", conn)
	}
