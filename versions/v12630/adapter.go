...
import (
	...
	"github.com/TheMMD-X/multitunnel/libs/gophertunnel/v12630/minecraft"
	"github.com/TheMMD-X/multitunnel/libs/gophertunnel/v12630/minecraft/protocol/login"
	"github.com/TheMMD-X/multitunnel/libs/gophertunnel/v12630/minecraft/protocol/packet"
)
...
func (network) DialContext(context.Context, string) (net.Conn, error) {
	return nil, errors.New("v12630: the multitunnel network cannot dial")
}

func (network) PingContext(context.Context, string) ([]byte, error) {
	return nil, errors.New("v12630: the multitunnel network cannot ping")
}

func (network) Listen(address string) (minecraft.NetworkListener, error) {
	f, ok := feeders.LoadAndDelete(address)
	if !ok {
		return nil, fmt.Errorf("v12630: no feeder registered under %q", address)
	}
	return f.(*feeder), nil
}
...
func Disconnect(conn net.Conn, message string) error {
	...
	if !ok {
		return fmt.Errorf("v12630: disconnect: unexpected connection type %T", conn)
	}
